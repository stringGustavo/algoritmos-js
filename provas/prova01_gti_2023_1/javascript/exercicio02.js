...
function exibir () {
    ...
    if(primeiroNumero > 0 && segundoNumero > 0 && terceiroNumero > 0) {
        div.innerHTML =  `<p>Os números digitados foram: ${primeiroNumero}, ${segundoNumero} e ${terceiroNumero}</p>`;
        div.innerHTML += `<p>O maior número é: ${maiorNumeroDigitado}</p>`;
        div.innerHTML += `<p>O menor número é: ${menorNumeroDigitado}</p>`;
    } else {
